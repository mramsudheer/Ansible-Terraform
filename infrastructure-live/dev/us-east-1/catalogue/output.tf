output "app_details" {
  description = "Detailed information for all deployed application instances"
  value = {    
    for app_name, instance in aws_instance.this : app_name => {
      instance_id    = instance.id
      private_ip     = instance.private_ip
      security_group = nonsensitive(instance.vpc_security_group_ids)  # Assuming one SG per instance for simplicity
      #value = aws_instance.catalogue_instnace.vpc_security_group_ids
      r53_record     = aws_route53_record.apps[app_name].name
    }
  }
}
